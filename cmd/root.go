...
func init() {
	...
	pf.DurationVar(&maxReadTime, "max-read-time", 600*time.Second, "maximum duration for reading the entire request, including the body. A zero or negative value means there will be no timeout")
	pf.DurationVar(&maxWriteTime, "max-write-time", 600*time.Second, "maximum duration before timing out writes of the response. It is reset whenever a new request's header is read")
	pf.DurationVar(&opts.MaxRequestTime, "max-request-time", time.Minute, "maximum duration for the entire request (evaluation, building and running). A zero or negative value means there will be no timeout")
	pf.DurationVar(&opts.MaxEvalTime, "max-eval-time", time.Minute, "maximum duration for the evaluation phase. A zero or negative value means there will be no timeout")
	pf.DurationVar(&opts.MaxBuildTime, "max-build-time", time.Minute, "maximum duration for the build phase. A zero or negative value means there will be no timeout")
	pf.DurationVar(&opts.MaxRunTime, "max-run-time", time.Minute, "maximum duration for the run phase. A zero or negative value means there will be no timeout")
